...
class Log(object):
    def __init__(self):
        logging.basicConfig(level=logging.INFO, filemode='w', filename='4-weighted-reciprocal.txt', format='%(message)s')
    ...
    @staticmethod
    def info(msg):
        logging.info(f'{datetime.now().strftime("%b%d/%y, %H:%M:%S.%f")} - reciprocal: {msg}')
...
def subprocess(start, number, total_lock, accurate_lock):
    ...
    for m in range(start + 1, start + number + 1):
        ...
        for k in range(k_range):
            dist = [0] * 10
            for i in range(k + 1):
                dist[predict[i][1]] += 800000 / predict[i][0]
