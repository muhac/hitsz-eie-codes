...
class Log(object):
    def __init__(self):
        logging.basicConfig(level=logging.INFO, filemode='w', filename='5-weighted-exponential.txt', format='%(message)s')
    ...
    @staticmethod
    def info(msg):
        logging.info(f'{datetime.now().strftime("%b%d/%y, %H:%M:%S.%f")} - exponential: {msg}')
