...
class Log(object):
    def __init__(self):
        logging.basicConfig(level=logging.INFO, filemode='w', filename='7-condensed-noise.txt', format='%(message)s')
...
log.info('Read Files')
with open('6-noise.json') as f_obj:
    data = json.load(f_obj)
train_data = []
for i in range(len(data)):
    train_data.append([np.array(data[i][0]), data[i][1]])
tmp = train_data.pop()
...
with open('7-condensed-noise.json', 'w') as f:
