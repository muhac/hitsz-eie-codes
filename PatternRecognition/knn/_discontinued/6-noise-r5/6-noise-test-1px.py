...
class Log(object):
    def __init__(self, mode):
        # 记录日志并且打印在控制台
        logging.basicConfig(level=logging.INFO, filemode=mode, filename='6-noise-test-1px.txt', format='%(message)s')
    ...
    def info(msg):
        # 写日志
        logging.info(f'{datetime.now().strftime("%b%d/%y, %H:%M:%S.%f")} - noise 1px: {msg}')
...
def difference(x1, x2):
    diff_0 = np.sum((x1[1:27, 1:27] - x2[1:27, 1:27]) ** 2)
    diff_r = np.sum((x1[1:27, 1:27] - x2[1:27, 2:28]) ** 2)
    diff_d = np.sum((x1[1:27, 1:27] - x2[2:28, 1:27]) ** 2)
    diff_l = np.sum((x1[1:27, 1:27] - x2[1:27, 0:26]) ** 2)
    diff_u = np.sum((x1[1:27, 1:27] - x2[0:26, 1:27]) ** 2)
    s = min(diff_0, diff_r, diff_d, diff_l, diff_u)
    return s
...
if __name__ == '__main__':
    ...
    start = Log('w')
    start.info(f'{datetime.now().strftime("%b%d/%y, %H:%M:%S.%f")} - noise 1px: Read Files...')
    k_range = 20                  # 遍历 k 最大值
    ...
    start.info(f'{datetime.now().strftime("%b%d/%y, %H:%M:%S.%f")} - noise 1px: {number_of_trains} training samples')
