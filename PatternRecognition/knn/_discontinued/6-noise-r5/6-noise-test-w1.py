...
if __name__ == '__main__':
    ...
    start = Log('w')
    start.info(f'{datetime.now().strftime("%b%d/%y, %H:%M:%S.%f")} - noise w1: Read Files...')
    k_range = 20                  # 遍历 k 最大值
    ...
    start.info(f'{datetime.now().strftime("%b%d/%y, %H:%M:%S.%f")} - noise w1: {number_of_trains} training samples')
