...
class Log(object):
    def __init__(self, mode):
        # 记录日志并且打印在控制台
        logging.basicConfig(level=logging.INFO, filemode=mode, filename='6-noise-test-w2.txt', format='%(message)s')
    ...
    def info(msg):
        # 写日志
        logging.info(f'{datetime.now().strftime("%b%d/%y, %H:%M:%S.%f")} - noise w2: {msg}')
...
def subprocess(q, train, test, k):
    for test_x, test_y in test:  # 对每一测试样本：
        ...
        for t in range(k):        # 遍历 k
            ...
            dist = [0] * 10
            for i in range(k + 1):
                dist[predict[i][1]] += math.exp(- (predict[i][0] / 800000) ** 2 / 2)
...
if __name__ == '__main__':
    ...
    start = Log('w')
    start.info(f'{datetime.now().strftime("%b%d/%y, %H:%M:%S.%f")} - noise w2: Read Files...')
    k_range = 20                  # 遍历 k 最大值
    ...
    start.info(f'{datetime.now().strftime("%b%d/%y, %H:%M:%S.%f")} - noise w2: {number_of_trains} training samples')
