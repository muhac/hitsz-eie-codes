...
class Log(object):
    def __init__(self, mode):
        # 记录日志并且打印在控制台
        logging.basicConfig(level=logging.INFO, filemode=mode, filename='6-noise-test.txt', format='%(message)s')
...
if __name__ == '__main__':
    ...
    start = Log('w')
    start.info(f'{datetime.now().strftime("%b%d/%y, %H:%M:%S.%f")} - noise: Read Files...')
    k_range = 20                  # 遍历 k 最大值
    ...
    start.info(f'{datetime.now().strftime("%b%d/%y, %H:%M:%S.%f")} - noise: {number_of_trains} training samples')
