...
class Log(object):
    def __init__(self):
        logging.basicConfig(level=logging.INFO, filemode='w', filename='knn-1.5.txt', format='%(message)s')
...
def difference(x1, x2):
    diff = np.sum((x1[4:24, 4:24] - x2[4:24, 4:24]) ** 2)
    return diff
...
def subprocess(start, number, lk):
    ...
    for m in range(start + 1, start + number + 1):
        ...
        y = np.argmax(np.bincount([predict[i][1] for i in range(k)]))
        result = y == test_y
        ...
        log.info(f'     Sample: {m:04}: {test_y}->{y} = {result:1}   - {count_accurate}/{count_total}')
