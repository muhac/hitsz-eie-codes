...
experiment (bool enableCtsRts, std::string wifiManager)
{
  ...
  lossModel->SetLoss (nodes.Get (0)->GetObject<MobilityModel> (),
                      ...
                      50); // set symmetric loss 0 <-> 2 to 50 dB
  lossModel->SetLoss (nodes.Get (0)->GetObject<MobilityModel> (),
                      nodes.Get (3)->GetObject<MobilityModel> (),
                      50); // set symmetric loss 0 <-> 3 to 50 dB
   ...
   * for \bugid{388} and \bugid{912}
   */
  onOffHelper.SetAttribute ("DataRate", StringValue ("3002200bps"));
  ...
  echoClientHelper.SetAttribute ("MaxPackets", UintegerValue (18));
